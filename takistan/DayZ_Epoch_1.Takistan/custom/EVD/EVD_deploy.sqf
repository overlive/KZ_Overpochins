		...
		if (_finished) then {
			...
			if(_removed_total == _tobe_removed_total) then {
				...
				_object addEventHandler ["GetIn",{
					_nil = [nil,(_this select 2),"loc",rTITLETEXT,"Warning: This vehicle will disappear on server restart! - Se você desmontar a caixa, os itens dentro dela irão sumir! CUIDADO!","PLAIN DOWN",15] call RE;
				}];
