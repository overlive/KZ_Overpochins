...
//Submarine Submerging
	if (typeOf _vehicle == "ori_submarine" && surfaceIsWater position player) then {
		  ...
		  if ((_vehicle animationPhase "sink") == 1) then {
			if (sub_Up < 0) then {
					sub_Up = _vehicle addAction ["Rise","scripts\sub_UPDown.sqf",0,5,false,true];
				};   
		   } else {
			   if (sub_down < 0) then {
					sub_Down = _vehicle addAction ["Submerge","scripts\sub_UPDown.sqf",1,5,false,true];
			};
		 };
	} else {
	 ...
	 if (s_player_plotManagement < 0) then {
		...
		s_player_plotManagement = player addAction ["<t color='#0059FF'>Manage Plot</t>", "plotManagement\initPlotManagement.sqf", [], 5, false];
		 ...
		 if (s_player_maintain_area < 0) then {
		  	s_player_maintain_area = player addAction [format[], "\z\addons\dayz_code\actions\maintain_area.sqf", "maintain", 5, false];
		 	s_player_maintain_area_preview = player addAction [format[], "\z\addons\dayz_code\actions\maintain_area.sqf", "preview", 5, false];
		 };
	 } else {
	...
	*/
	...
	_name = getText (configFile >> "CfgVehicles" >> _typeOfCursorTarget >> "displayName");
		if (s_player_special_backpack < 0) then {		 
			s_player_special_backpack = player addAction [format[(localize "STR_DAYZ_CODE_1"),_name], "custom\special_backpacks\backpack_pickup.sqf",["CfgVehicles",_typeOfCursorTarget,_cursorTarget], 20, true, true];
		};
	} else {
	...
	if (_isMan && !_isPZombie && _traderType in serverTraders) then {
		...
		if (s_player_parts_crtl < 0) then {
			...
			if((_traderMenu select 2) == "friendly") then {
				...
				_humanity_required="greater than -5000";
			};
			if((_traderMenu select 2) == "hostile") then {
				_low_high = "high";
				_humanity_logic = (_humanity > -5000);
				_humanity_required="less than -5000";
			};
			if((_traderMenu select 2) == "cvarmed") then {
				_humanity_logic = (_humanity < 5000);
				_humanity_required="greater than 5000";
			};
			if((_traderMenu select 2) == "mvu") then {
				_humanity_logic = (_humanity < 7500);
				_humanity_required="greater than 7500";
			};			
			if((_traderMenu select 2) == "mva") then {
                _humanity_logic = (_humanity < 10000);
				_humanity_required="greater than 10000";
            };
			if((_traderMenu select 2) == "t2") then {
                _humanity_logic = (_humanity < 10000);
				_humanity_required="greater than 10000";
            };	
			if((_traderMenu select 2) == "t3") then {
                _humanity_logic = (_humanity < 15000);
				_humanity_required="greater than 15000";
            };	
			if((_traderMenu select 2) == "hu") then {
                _humanity_logic = (_humanity < 15000);
				_humanity_required="greater than 15000";
            };	
			if((_traderMenu select 2) == "ha") then {
                _humanity_logic = (_humanity < 25000);
				_humanity_required="greater than 25000";
            };			
			if(_humanity_logic) then {
				_cancel = player addAction [format["Humanity required to trade: %1",_humanity_required], "\z\addons\dayz_code\actions\trade_cancel.sqf",["na"], 0, true, false, "",""];
